...
.. admonition:: Under construction
   ...
   Redis: overview (limitations, compare to sql, performance, reliability), redis configuration (max-memory policy, client timeout, persistence, replication, high availability, failover), hangfire configuration (connection string, invisibility timeout, prefix, database, ssl, cluster, sentinel, multiple endpoints, dashboard metrics), semver and schema, implementation details (queue priorities, sliding distributed locks, fetch algorithm), memory, connections.
